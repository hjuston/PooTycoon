...
public class BigInteger
{
    ...
    private void Construct(string digits, int radix)
    {
        ...
        for (int idx = digits.Length - 1; idx >= nDigits; idx--)
        {
            ...
            result += (multiplier * new BigInteger(d));
            multiplier *= new BigInteger(radix);
        }
    ...
    public static BigInteger operator *(BigInteger leftSide, float rightSide)
    {
        int calosci = (int)Math.Truncate(rightSide);
        int dziesietne = (int)Math.Round((rightSide - Math.Truncate(rightSide)) * 100);

        BigInteger pomnozone = leftSide * new BigInteger(100);
        BigInteger pomnozonePrzezDziesietne = pomnozone * new BigInteger(dziesietne);

        BigInteger resultByCalosci = leftSide * new BigInteger(calosci);
        BigInteger resultByDziesitne = pomnozonePrzezDziesietne / new BigInteger(10000);
        BigInteger result = resultByCalosci + resultByDziesitne;
